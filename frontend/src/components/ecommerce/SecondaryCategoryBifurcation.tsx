...
interface SecondaryCategoryBifurcationProps {
  selectedUserId: string;
}

export default function SecondaryCategoryBifurcation({
  selectedUserId,
}: SecondaryCategoryBifurcationProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const url = `${BACKEND_URL}/dashboard/secondary-category-bifurcation`;
        const params =
        ...
        if (response.data.success) {
          ...
          setError("Failed to fetch secondary category data");
        }
      } catch (err) {
        console.error("Error fetching secondary category bifurcation:", err);
        setError("Error loading secondary category data");
      } finally {
        setLoading(false);
      }
    };
